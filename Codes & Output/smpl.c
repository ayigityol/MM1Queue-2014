...
/*                                                                    */
/* DZD (28/11/14) - changed static FILE * to -> FILE *				  */
/*                  FILE *display=stdout  to -> FILE *display,	      */
/*                  FILE *opf=stdout;     to -> FILE *opf;		      */                     
/**********************************************************************/
#include "smpl.h"
#include <stdlib.h>
...
/* can not initialize GLOBALs here with stdout since stdout 
  is NOT a constant yet! 
*/
FILE                 /* DZD(28/11/14):static keyword removed */
...
void reportf(void)
  {
    int f;
	opf = fopen("samples.txt","a+");    ;      /* DZD(28/11/14) */	
    if ((f=fchn)==0)
      ...
      while(f && lft--)
        {
          ...
          fprintf(opf," %-17s%6.4f %10.6f %13.3f %11d %9d %7d\n",
